...
$stmt = $pdo -> prepare("SELECT ob.kind,ob.name,ob.email,ob.url,ob.text,obp.price,obp.Delivery_fee,obp.exchange_rate,obp.my_url FROM
  order_before AS ob inner join  order_before_price AS obp ON ob.order_id = obp.order_before_pk where ob.order_num = :order_num order by ob.order_num asc");
...
<?php
        ...
        function SumPrice()  //상품가격 자동합계
        {
          ...
          if(count > 0){
            ...
            document.getElementById('Repackaging').value = RepackagingV;
             ...
             document.getElementById('JPYV').value = String(jpyV)+"円";
    ...
    <div class="container">
      <button class="btn btn-primary" onclick="myCaptureFunction()">画像で保存</button> <!--캡쳐 작동시키는 버튼이 최상단에 와야지만 작동함..왠진 모름-->
    </div>
            ...
            <table class="table table-bordered" id="urlList">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td><input type="text" class="form-control" id="price<?php echo $i; ?>"  name="price[]"  readonly required  value="<?php echo $strPrice[$i];?>"></td>
                  <td><input type="text" class="form-control" id="DFIK<?php echo $i; ?>"   name="DFIK[]"  readonly  value="<?php echo $strDF[$i];?>"></td>
                </tr>

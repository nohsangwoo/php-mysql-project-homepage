...
<!--메일보내기-->
    ...
    $query = "UPDATE order_before_price_second SET  delivery_way  = :delivery_way,name  = :name,email  = :email,address  = :address,phone  = :phone,text  = :text,s_option  = :s_option  WHERE order_before_pk = :order_before_pk ";
    $st = $pdo->prepare($query);
    $st->bindparam(":order_before_pk", $order_before_pk);
    $st->bindparam(":delivery_way", $delivery_way);
    $st->bindparam(":name", $name);
    ...
    header('Location: ./re_four_list.php');
    ?>
